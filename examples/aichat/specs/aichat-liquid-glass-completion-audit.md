...
| AppleNativeInterleave renderer requirements | `APPLE-NATIVE-INTERLEAVE-RENDERER-REQUIREMENTS.md`; Step 96 lower scene pass probe; Step 98 fail verdict; Step 172 interleave hierarchy probe; Step 173 transparent-overlay interleave runnable; Step 174 interleave screenshot routing; Step 175 foreground alpha audit; Step 176 system screenshot blocker; Step 177 diagnostic overlay runnable; Step 178 diagnostic interleave visual pass | Requirements and prototype probes exist. Step 172 proves the AppKit hierarchy can be `lower Metal sibling -> native glass container -> primary Metal view`, so the remaining production blocker is the Makepad render split, not basic AppKit sibling ordering. Step 173 adds a minimal Studio runnable that combines lower scene, native glass, upper primary, and transparent panel overlay for the next manual visual gate. Step 174 fixes the Studio screenshot gate so lower-scene diagnostic passes no longer consume default screenshot requests before the primary foreground pass. Step 175 proves the apparent black foreground screenshot background is mostly alpha=0, so the current probe is close to foreground-only at the framebuffer level. Step 176 records that local system screenshot automation produced a black frame and cannot close native material visibility. Step 177 adds a stronger diagnostic lower-scene profile runnable for manual visual verdict and confirms its foreground screenshot remains mostly alpha=0. Step 178 records a user-provided system screenshot where broad interior lower-scene glass is visible, passing the diagnostic interleave visual hypothesis. Production `AppleNativeInterleave` is still incomplete until the diagnostic lower scene becomes a real aichat lower/foreground split. |
