...
| Full native interior glass | `aichat-liquid-glass-v4.2-route-decision.md`; `aichat-liquid-glass-step-96-aichat-lower-scene-pass-probe.spec`; `aichat-liquid-glass-step-98-native-interleave-fail-verdict.spec`; `aichat-liquid-glass-step-171-underlay-edge-only-verdict.md`; Step 96 proves a dedicated aichat `DrawPassSurfaceRole::LowerScene` pass is rendered and routed to the lower scene surface, but Step 98 records the user-visible result as gray/grid only, with no transparency or recognizable blur/refraction/liquid distortion. Step 171 records that the current `AppleNativeUnderlay` composition is expected to show native material mostly at edges because Makepad paints interior readability/content layers above the native glass. | Not complete: AppleNativeInterleave is rejected as the current production route and remains a prototype; AppleNativeUnderlay is edge/substrate native glass, not full native interior Liquid Glass. |
...
| AppleNativeInterleave renderer requirements | `APPLE-NATIVE-INTERLEAVE-RENDERER-REQUIREMENTS.md`; Step 96 lower scene pass probe; Step 98 fail verdict; Step 172 interleave hierarchy probe; Step 173 transparent-overlay interleave runnable; Step 174 interleave screenshot routing; Step 175 foreground alpha audit; Step 176 system screenshot blocker; Step 177 diagnostic overlay runnable | Requirements and prototype probes exist. Step 172 proves the AppKit hierarchy can be `lower Metal sibling -> native glass container -> primary Metal view`, so the remaining production blocker is the Makepad render split, not basic AppKit sibling ordering. Step 173 adds a minimal Studio runnable that combines lower scene, native glass, upper primary, and transparent panel overlay for the next manual visual gate. Step 174 fixes the Studio screenshot gate so lower-scene diagnostic passes no longer consume default screenshot requests before the primary foreground pass. Step 175 proves the apparent black foreground screenshot background is mostly alpha=0, so the current probe is close to foreground-only at the framebuffer level. Step 176 records that local system screenshot automation produced a black frame and cannot close native material visibility. Step 177 adds a stronger diagnostic lower-scene profile runnable for manual visual verdict and confirms its foreground screenshot remains mostly alpha=0. Current visual verdict still rejects this as production full native glass. |
...
- full native interior Liquid Glass is not implemented; Step 96 proves lower
  ...
  automation returned a black frame, so the current gate is blocked on user
  manual verdict or a usable system-composition screenshot. Step 177 adds a
  stronger diagnostic overlay target to make that verdict easier.
...
3. Collect the Step 176/177 visual verdict for
   `makepad-example-aichat-macos-native-clear-interleave-diagnostic-overlay`:
   either user manual confirmation or a usable system-composition screenshot.
