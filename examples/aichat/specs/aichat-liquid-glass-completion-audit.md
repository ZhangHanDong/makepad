...
| AppleNativeInterleave renderer requirements | `APPLE-NATIVE-INTERLEAVE-RENDERER-REQUIREMENTS.md`; Step 96 lower scene pass probe; Step 98 fail verdict; Step 172 interleave hierarchy probe | Requirements and a prototype probe exist. Step 172 proves the AppKit hierarchy can be `lower Metal sibling -> native glass container -> primary Metal view`, so the remaining production blocker is the Makepad render split, not basic AppKit sibling ordering. Current visual verdict still rejects this as production full native glass. |
...
- full native interior Liquid Glass is not implemented; Step 96 proves lower
  ...
  lower scene and transparent upper foreground passes.
...
2. Keep complete aichat interior glass on ShaderBackdrop or a hybrid route. A
   ...
   UI on the primary Metal surface.
