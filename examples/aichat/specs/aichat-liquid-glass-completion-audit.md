...
- Step 138 adds stable iOS transient popup unsupported logging. When
  ...
  aarch64-apple-ios --release` passes. iOS test-target `--no-run` remains
  blocked by an unrelated `live_reload.rs` test compile issue, so this is
  compile/observability evidence, not runtime UIKit proof.
...
- Phase I popup/modal policy is defined in
  ...
  implementation and runtime validation, and modal windows remain incomplete.
