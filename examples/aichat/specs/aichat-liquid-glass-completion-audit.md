...
| AppleNativeInterleave renderer requirements | `APPLE-NATIVE-INTERLEAVE-RENDERER-REQUIREMENTS.md`; Step 96 lower scene pass probe; Step 98 fail verdict; Step 172 interleave hierarchy probe; Step 173 transparent-overlay interleave runnable; Step 174 interleave screenshot routing; Step 175 foreground alpha audit; Step 176 system screenshot blocker | Requirements and prototype probes exist. Step 172 proves the AppKit hierarchy can be `lower Metal sibling -> native glass container -> primary Metal view`, so the remaining production blocker is the Makepad render split, not basic AppKit sibling ordering. Step 173 adds a minimal Studio runnable that combines lower scene, native glass, upper primary, and transparent panel overlay for the next manual visual gate. Step 174 fixes the Studio screenshot gate so lower-scene diagnostic passes no longer consume default screenshot requests before the primary foreground pass. Step 175 proves the apparent black foreground screenshot background is mostly alpha=0, so the current probe is close to foreground-only at the framebuffer level. Step 176 records that local system screenshot automation produced a black frame and cannot close native material visibility. Current visual verdict still rejects this as production full native glass. |
...
- full native interior Liquid Glass is not implemented; Step 96 proves lower
  ...
  manual verdict or a usable system-composition screenshot.
...
3. Collect the Step 176 visual verdict for
   `makepad-example-aichat-macos-native-clear-interleave-transparent-overlay`:
