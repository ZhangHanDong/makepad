...
| Full native interior glass | `aichat-liquid-glass-v4.2-route-decision.md`; `aichat-liquid-glass-step-96-aichat-lower-scene-pass-probe.spec`; `aichat-liquid-glass-step-98-native-interleave-fail-verdict.spec`; `aichat-liquid-glass-step-171-underlay-edge-only-verdict.md`; Step 178 diagnostic interleave visual pass; Step 181 production lower-scene detail; Step 182 guarded backend; Step 183 app log naming | Not complete: diagnostic interleave now visually proves broad interior native glass, and guarded production preview reaches State 4 with a grid-free detailed lower scene. The production preview still needs a manual system-composited visual verdict before it can be promoted beyond guarded preview. |
| AppleNativeInterleave backend name | `aichat-liquid-glass-step-48-native-interleave-backend-guard.spec`; `aichat-liquid-glass-step-99-interleave-fallback-to-shader-backdrop.spec`; Step 182 guarded backend; Step 183 app log naming | Guarded production preview exists. Default unguarded requests still fall back with the Step 98 warning; explicit `AICHAT_ENABLE_APPLE_NATIVE_INTERLEAVE=production-preview` runs the State 4 preview and logs `app-substrate=apple-native-interleave`. |
| AppleNativeInterleave renderer requirements | `APPLE-NATIVE-INTERLEAVE-RENDERER-REQUIREMENTS.md`; Step 96 lower scene pass probe; Step 98 fail verdict; Step 172 interleave hierarchy probe; Step 173 transparent-overlay interleave runnable; Step 174 interleave screenshot routing; Step 175 foreground alpha audit; Step 176 system screenshot blocker; Step 177 diagnostic overlay runnable; Step 178 diagnostic interleave visual pass; Step 179 production preview; Step 181 production lower-scene detail; Step 182 guarded backend; Step 183 app log naming | Requirements and guarded preview probes exist. Step 172 proves the AppKit hierarchy can be `lower Metal sibling -> native glass container -> primary Metal view`; Step 178 proves the diagnostic interleave visual hypothesis; Step 181 removes the diagnostic grid while keeping lower-scene detail; Step 182 makes the route explicitly runnable as guarded `AppleNativeInterleave`; Step 183 fixes app-facing naming. Production `AppleNativeInterleave` remains incomplete until build `[37]` or a later guarded build receives a manual system-composited visual pass. |
