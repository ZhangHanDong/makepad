...
| Native interactive controls | `APPLE-NATIVE-BUTTON-GLASS-RESEARCH.md`; `APPLE-NATIVE-INTERACTIVE-CONTROLS-POLICY.md`; Step 100 closes the first hit-test ownership decision: explicit native controls use direct AppKit/UIKit hit testing, actions bridge back into Makepad, and ordinary glass panels stay passthrough | Research and hit-test ownership decision landed; native interactive controls are not implemented. |
...
- Native interactive controls are not implemented; Step 100 only closes the
  first hit-test ownership decision for a later prototype.
