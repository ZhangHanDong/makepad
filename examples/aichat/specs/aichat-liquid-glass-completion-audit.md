...
- Native interactive controls are not implemented; Step 100 closes the first
  ...
  and Step 104 adds opt-in Button descriptor export, but current OS backends
  still ignore native control batches. Step 105 adds the shared activation
  event path, but no backend posts it yet. Step 106 adds button labels to
  descriptors, but no backend creates native controls from them yet.
