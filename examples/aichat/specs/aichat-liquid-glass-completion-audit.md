...
| Phase H advanced behavior | `APPLE-NATIVE-ADVANCED-BEHAVIOR-GATES.md`; Step 53 adds explicit fullscreen native fallback/suppression; Step 54 adds `native-display-change` backing-scale probe; v4 spec lists animated spacing, scroll edge glass, fullscreen/multi-display, inactive-window behavior | Phase H is gate-defined only; fullscreen has explicit fallback; multi-display support remains unproven. |
