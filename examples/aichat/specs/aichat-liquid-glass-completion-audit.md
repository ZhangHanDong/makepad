...
- Native interactive controls are not implemented; Step 100 closes the first
  ...
  labels to descriptors, but no backend creates native controls from them yet.
  Step 107 adds macOS validation/logging for control batches, but macOS still
  reports `installer-not-implemented` and non-macOS backends remain no-op.
