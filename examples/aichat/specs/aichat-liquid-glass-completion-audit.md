...
- Native interactive controls are not implemented; Step 100 closes the first
  ...
  styling is unproven, Step 109's aichat native-control probe still needs
  Studio/runtime validation, UIKit controls are not installed, accessibility
  ownership is unresolved, and non-macOS backends remain no-op.
