...
impl CxSplashVmExt for Cx {
    ...
    fn with_script_vm_id<R>(&mut self, vm_id: SplashVmId, f: impl FnOnce(&mut ScriptVm) -> R) -> R {
        ...
        // Fork guard: the VM may already be gone (Splash dropped, or not yet
        // allocated) when a timer/async callback fires. Do NOT panic inside
        // with_isolate_installed; fall back to the main vm — a harmless no-op
        // for body/callback lookups.
        if !self
            .global::<CxWidgetAsync>()
            .isolated_vms
            .vms
    ...
    fn with_script_vm_id_thread<R>(
        ...
        // Fork guard: see with_script_vm_id — fall back instead of panicking
        // when the isolate is already gone.
        if !self
            .global::<CxWidgetAsync>()
            .isolated_vms
            .vms
