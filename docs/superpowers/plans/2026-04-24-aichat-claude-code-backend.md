...
**Architecture:** Implement a new session-based `ClaudeCodeCliAgent` under `old/studio/ai/src/backends/` and keep tmux out of scope. The backend should be testable without invoking real Claude by splitting availability/command construction and stream-json parsing into pure helper functions. aichat only adds a backend enum variant, label, detection, and creation path.
...
- `old/studio/ai/src/backends/claude_code_cli.rs`: new backend implementation, command builder, availability helper, stream-json parser, and unit tests.
- `old/studio/ai/src/backends/mod.rs`: module declaration and public re-export.
- `old/studio/ai/src/lib.rs`: no change expected unless public exports require adjustment.
- `old/studio/ai/Cargo.toml`: no new dependency expected; change only if unavoidable.
...
**Files:**
- Create: `old/studio/ai/src/backends/claude_code_cli.rs`
- Modify: `old/studio/ai/src/backends/mod.rs`

- [ ] **Step 1: Add a failing public export test**

Create `old/studio/ai/src/backends/claude_code_cli.rs` with a test module containing:
...
**Files:**
- Modify: `old/studio/ai/src/backends/claude_code_cli.rs`
